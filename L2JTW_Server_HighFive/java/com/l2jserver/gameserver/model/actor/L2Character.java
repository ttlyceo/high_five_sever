	...
	protected void onHitTimer(L2Character target, int damage, boolean crit, boolean miss, boolean soulshot, byte shld)
	{
		...
		if (!isAttackAborted())
		{
			...
			if (!miss && damage > 0)
			{
				...
				if (!target.isInvul()) // Do not reflect if weapon is of type bow or target is invunlerable
				{
					...
					if (!target.isRaid() || getActingPlayer() == null || getActingPlayer().getLevel() <= target.getLevel() + 8)
					{
						...
						if (reflectPercent > 0)
						{
							...
							if(!isBow)
							{
								reflectedDamage = reflectedDamage / 2;
							}
				...
				if (true) //if (!isBow) // Do not absorb if weapon is of type bow
				{
