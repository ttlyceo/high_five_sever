...
:_check_copy
if exist "GS-Support.txt" goto _check_copy_next
if exist "DP-Support.txt" echo 在這個資料夾應該執行「DP快速編譯」才對！
if exist "DP-Support.txt" echo.
if exist "DP-Support.txt" pause
if exist "DP-Support.txt" goto _EXIT
...
FOR /F %%i IN (%temp%\check.txt) DO IF %%i GTR 0 echo 在這個資料夾應該執行「DP快速編譯」才對！
...
if not exist "2.GS*.cmd" goto _UPDATE
if exist "1.DP*.cmd" del "1.DP*.cmd"
if exist "1.CB*.cmd" del "1.CB*.cmd"
if exist "2.DP*.cmd" del "2.DP*.cmd"
...
if not exist "%ProgramFiles%\Java\jre7\bin\java.exe" goto _JAVA_ERR

:_check_ver
if not exist "%ProgramFiles%\Java\jdk1.7.*" goto _JAVA_ERR
...
if exist "%ProgramFiles%\Java\%ver%\lib\ant_184gs.txt" goto _JAVA_VER_NEXT
echo.>> "%ProgramFiles%\Java\%ver%\lib\ant_184gs.txt"
if not exist "%ProgramFiles%\Java\%ver%\lib\ant_184gs.txt" goto _NO_ACCESS
...
if exist "build\*.zip" echo ■■■■■■■■■■■
if exist "build\*.zip" echo ■ 　GS編譯成功！ 　■
if exist "build\*.zip" echo ■■■■■■■■■■■
echo.
...
:_JAVA_ERR
echo %ProgramFiles%\ 找不到 Java 7
echo.
echo 請先安裝 JAVA JDK 7 的最新版本，並且不要更改預設安裝位置
echo.
echo ■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
echo ■ 注意：嗨翻天 和 塔武提，必須安裝 JAVA JDK 7 (沙哈之裔 則是 JDK 8)  ■
echo ■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
echo.
pause
goto _EXIT
...
echo 無法在這個資料夾執行「編譯」
echo.
echo 請將 GS 更新到最新版本後，再試一次
echo.
pause
goto _EXIT

:_UPDATE
echo 請先將「工具-Java 7」內的 9 個檔案，複製到 GS/DP/CB 的資料夾
echo.
pause
goto _EXIT
